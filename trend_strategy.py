...
ЛОГИКА (v2 — вход «как у ликвидаций», ЗА N секунд до конца окна):
  ...
  7. Фильтр флета (опционально): сигнальное окно сравнивается с окном
     ПРЕДЫДУЩЕЙ свечи по трём метрикам — ликвидации ($), CVD-поток ($),
     изменение OI (%); у каждой свой порог и свой режим below/above.
     СРАБОТАЛ фильтр = рынок в противофазе → вход не пропускается, а
     ОТКРЫВАЕТСЯ В ОБРАТНУЮ сторону (и для шагов серии мартингейла тоже).
     Нет данных у источника — фильтрация молчит, вход как по свече.
...
STRATEGY_VERSION = "01.09.2026 #2 — следование за свечой с реверсом на " \
                   "флете (лика/CVD/OI, режим у каждого свой), FAK-вход, " \
                   "TP отложником/FAK, мартингейл ×N, своя статистика"
...
DEFAULTS = {
    ...
    # 0 — метрика выключена. У КАЖДОЙ метрики свой режим (комбинируются
    # свободно): below — сработал, ЕСЛИ МЕНЬШЕ N% (движение не подтвердилось
    # = флет); above — если БОЛЬШЕ N% (шторм/перегрев). Сработка = НЕ пропуск,
    # а вход в ОБРАТНУЮ сторону (тот же принцип и для шагов серии).
    "td_liq_prev_pct": "0",
...
def _ratio_note(name: str, ratio: float | None, pct: float, mode: str) -> str:
    ...
    if ratio is None:
        return f"{name}: нет данных (реверс при {arrow}{pct:g}%)"
    rs = "∞" if ratio == float("inf") else f"{ratio:.1f}%"
    return f"{name}: {rs} от пред. окна (правило: {arrow}{pct:g}%)"
...
async def _flat_check(session, c: dict, symbol: str, sig_start: int,
    ...
    Возвращает (ключ_счётчика|None, пояснение, ratio-для-лога). «Сработал»
    = включённая метрика попала в режим (below: меньше порога — флет;
    above: больше — перегрев): вход разворачивается в обратную сторону.
    Недостаточно данных — НЕ срабатывает (вход как по свече)."""
    fcfg = _flat_cfg(c)
    lp, lm = fcfg["liq"]
    cp, cm = fcfg["cvd"]
    op, om = fcfg["oi"]
...
async def _open_in_window(context, cid, session, c: dict, symbol: str,
    ...
    # относительно предыдущего (или, наоборот, шторм) — рынок в
    # противофазе: НЕ пропускаем, а входим в ОБРАТНУЮ сторону. Правило
    # одинаково и для первого входа, и для шагов серии мартингейла.
    rev_note = ""
    if sig_start and _flat_enabled(c):
        ...
        if f_key:
            ...
            rev_note = (f"реверс по метрике «{f_key.replace('td_stat_flat_', '')}» "
                        f"(все проверки: {f_note}) — вход против свечи")
            set_setting("td_last_scan", f"[{symbol}] {rev_note}")
    ...
    try:
        ...
        pass
    candle_dir = (ls.resolve_state(candle) if candle else None) or outcome
    ...
    rev_line = (f" → вход *{outcome}* (реверс)\n🔁 {ls.md_escape(rev_note)}\n"
                if rev_note else "\n")
    await _send(
        context, cid,
        f"{'🔁' if rev_note else '🕯'} "
        f"*{'ВХОД ПРОТИВ СВЕЧИ' if rev_note else 'ВХОД ЗА РЫНКОМ'}* "
        f"`{symbol}` {_dir_emoji(outcome)} {mode_emoji}\n\n"
        f"📊 {src_note or 'Свеча'}: *{candle_dir}* ({delta_pct:+.3f}%)"
        + rev_line +
...
def get_status_text() -> str:
    ...
    if any(pct > 0 for pct, _m in _fc.values()):
        ...
        L.append("🌫 Против.prev свечи (сработал = вход в обратную сторону): "
