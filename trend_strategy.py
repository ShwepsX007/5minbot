...
ЛОГИКА (простая, по ТЗ):
  1. В начале каждого окна (первые `td_entry_window_sec` секунд) бот
     смотрит на ПОСЛЕДНЮЮ ЗАКРЫТУЮ свечу этого же таймфрейма — ровно ту,
     по которой Polymarket уже рассчитал предыдущий рынок Up/Down:
        свеча вверх (close >= open) → покупаем UP текущего окна
        свеча вниз                  → покупаем DOWN текущего окна
     Это движение ЗА рынком: направление диктует только что закрывшаяся
     свеча, вход — в окно, которое она «породила».
  2. Вход — рыночным FAK-ордером (как и в стратегии ликвидаций):
     `place_market_order(..., order_type="FAK")`, сумма в USDC.
  3. Take-profit: цена нашего исхода дошла до `td_tp_cents` —
     ...
     FAK-ордером.
  4. Профит → серия закрыта (счётчик мартингейла обнуляется).
     Убыток → серия продолжается: в СЛЕДУЮЩЕМ окне бот снова смотрит
     на проигрышную свечу (она и есть «последняя закрытая») и заходит
     в её направлении лотом с мартингейлом: stake = base × mult^шаг.
     То есть правило направления единое и для первого входа, и для
     каждого шага отыгрыша.
  5. Если до конца окна candle пошёл ПРОТИВ нас (за `td_final_check_sec`
     секунд до закрытия) — позиция досрочно продаётся по рынку (FAK),
     убыток фиксируется и серия уходит в следующее окно (spасаем остаток,
     как в первой системе). Иначе позиция держится до расчёта рынка:
     выигрышные доли по исходу окна гасятся по 100¢.
...
DEFAULTS = {
    ...
    # Интервал входа: старт окна + delay .. window секунд. Позже —
    # поздно: цена уже уехала, матожидание «догоняющего» входа хуже.
    "td_entry_delay_sec": "5",
    "td_entry_window_sec": "120",
    ...
    "td_tp_mode": "auto",
    # Досрочное спасение, если свеча окна пошла против нас.
    "td_salvage_on": "1",
    "td_final_check_sec": "20",
...
def _mark_window(symbol: str, win_start: int, burn: bool = False,
                 inc_try: int = 0) -> None:
    """Обновляет state (перечитывая свежий — без гонок): отмечает окно
    обработанным (burn) и/или поднимает счётчик попыток (inc_try=max попыток,
    после которых окно выгорает)."""
    st = _load_state()
    changed = False
    if burn:
...
async def _skip_and_note(symbol: str, reason: str):
    ...
    log.info(f"trend: {symbol} — вход пропущен: {reason}")


async def _try_enter(context, cid, session, c: dict, symbol: str, cur_start: int,
                     age: float):
    """Один проход точки входа по одной монете (вход в текущее окно)."""
    import polymarket_trading as pt

    state = _load_state()
    if symbol in (state.get("positions") or {}):
        return  # позиция ещё жива — ведёт её job-скан
    tf = str(c.get("td_timeframe", "5m"))
    dur = ls.TF_SECONDS.get(tf, 300)

    # Одно решение на окно: не долбить рынок после успеха/осознанного пропуска.
    if int((state.get("last_window") or {}).get(symbol, -1)) == int(cur_start):
        return

    # Лимит одновременных позиций (только внутри этой стратегии).
    max_conc = max(1, _f_int(c, "td_max_concurrent", 1))
    if len(state.get("positions") or {}) >= max_conc:
        return

    # --- 1. Направление: последняя ЗАКРЫТАЯ свеча (предыдущее окно) ---
    prev_start = cur_start - dur
    candle = await _candle_for_window(session, symbol, tf, prev_start)
    if not candle:
        await _skip_and_note(symbol, f"нет данных свечи окна {prev_start} (ждём тик)")
        return
    outcome = ls.resolve_state(candle)
    if not outcome:
        # Пустая свеча без open/close — окно считаем отработанным,
        # чтобы не гонять попытки до бесконечности.
        _mark_window(symbol, cur_start, burn=True)
        await _skip_and_note(symbol, "свеча не даёт направление (нет open/close)")
        return

    # --- 2. Рынок текущего окна ---
    slug = ls.build_updown_slug(symbol, tf, cur_start, 0)
    info = pt.get_event_markets(slug)
    if not info or not info.get("markets"):
        # Рынок мог ещё не сгенерироваться — до 5 попыток, потом окно сгораем.
    ...
    # --- 4. Лот с мартингейлом ---
    # Перечитываем state: за сетевые вызовы выше серия могла измениться
    # (например, шаг только что проиграл и серия подросла).
    ...
    if not demo:
        ...
        if not order_ok:
            # Стакан/сеть могут поправиться к следующему тику — до 3 попыток.
            _mark_window(symbol, cur_start, inc_try=3)
    ...
    # --- 7. Пишем позицию (перечитаем state — защита от гонки) ---
    state = _load_state()
    ...
    pos = {
        ...
        "series": series,
        "window_start": int(cur_start),
        "window_end": int(cur_start) + dur,
        "open_ts": now,
        "is_demo": is_demo_flag,
        "market_question_raw": (m.get("question") or ""),
        "prev_candle": {"open": candle.get("open"), "close": candle.get("close"),
                        "t": candle.get("t"), "src": candle.get("src")},
    }
    state["positions"][symbol] = pos
    state.setdefault("last_window", {})[symbol] = int(cur_start)
    ...
    try:
        ...
        pass
    win_end_str = datetime.fromtimestamp(pos["window_end"], tz=timezone.utc).strftime("%H:%M:%S UTC")
    ...
    await _send(
        context, cid,
        f"🕯 *ВХОД ПО СВЕЧЕ* `{symbol}` {_dir_emoji(outcome)} {mode_emoji}\n\n"
        f"📊 Предыдущая свеча: *{outcome}* ({delta_pct:+.3f}%) — идём за рынком\n"
        f"📈 `{slug}`\n"
        f"💵 Лот: *${pos['stake']:.2f}* по {entry_cents}¢ "
        f"({shares:g} долей, FAK){ser_note}\n"
        f"🏁 TP: *{tp_cents}¢* | ⏰ Окно до *{win_end_str}*",
    )
    log.info(f"trend: ✅ вход {symbol} {outcome} ${pos['stake']:.2f} "
             f"@ {entry_cents}¢ (серия {series})")
...
async def _manage_position(context, cid, session, c: dict, state: dict,
    ...
    # --- 4. Спасение: свеча против нас в концовке окна ---
    if (str(c.get("td_salvage_on", "1")) == "1"
            and time_left <= max(3, _f_int(c, "td_final_check_sec", 20))):
        wc = await _candle_for_window(session, symbol, tf, pos.get("window_start") or
                                      (now - (dur - time_left)), force=True)
        candle_now = ls.resolve_state(wc)
        if candle_now and candle_now != outcome:
            delta_pct = 0.0
            try:
                ...
                pass
            log.info(f"trend: ❌ {symbol} свеча {candle_now} против {outcome} "
                     f"({delta_pct:+.3f}%) за {int(time_left)}с до конца — спасаем остаток")
            ...
            pos["salvage_sold"] = 1 if ok_sell else 0
            await _settle(context, cid, c, state, symbol, pos,
                          win=round(realized - stake, 2) > 0,
                          close_price=close_px if ok_sell else 0,
                          reason=(f"свеча {candle_now} против {_dir_emoji(outcome)} "
                                  f"({delta_pct:+.3f}%), продано по стакану "
                                  f"({'отложник' if mode == 'limit' else 'FAK'})"
                                  if ok_sell else
                                  f"свеча {candle_now} против {_dir_emoji(outcome)}, "
                                  f"остаток продать не удалось (пыль)"),
...
async def _scan_entry_impl(context):
    ...
    delay = max(0.0, _f_num(c, "td_entry_delay_sec", 5.0))
    window = max(delay + 2.0, _f_num(c, "td_entry_window_sec", 120.0))
    if age < delay or age > window:
        return  # не в интервале входа — спим до следующей границы окна

    async with aiohttp.ClientSession() as session:
        for symbol in symbols:
            try:
                await _try_enter(context, cid, session, c, symbol, cur_start, age)
            except Exception as e:
...
def get_status_text() -> str:
    ...
    cur_start = int(now // dur) * dur
    age = now - cur_start
    in_window = (_f_num(c, "td_entry_delay_sec", 5.0) <= age
                 <= max(0, _f_num(c, "td_entry_window_sec", 120.0)))
    ...
    L.append(f"🕯 Вход: за направлением *последней закрытой свечи* ({src}) — "
             f"{_f_num(c, 'td_entry_delay_sec', 5):g}…{_f_num(c, 'td_entry_window_sec', 120):g}с "
             f"от старта окна | сейчас {age:.0f}с от старта"
             f"{' ✅' if in_window else ' 💤'}")
    cap = _f_int(c, "td_entry_cap_cents", 0)
    ...
    L.append(f"🏁 TP: *{_f_int(c, 'td_tp_cents', 80)}¢* "
             ...
             f"🛡 Спасение: {'вкл за ' + str(_f_int(c, 'td_final_check_sec', 20)) + 'с до конца окна' if str(c.get('td_salvage_on', '1')) == '1' else 'выкл'}")
    ...
    for sym in symbols:
        ...
        if pos:
            ...
            L.append(f"🟢 `{sym}` свободна | жду старт окна (вход "
                     f"{_f_num(c, 'td_entry_delay_sec', 5):g}–"
                     f"{_f_num(c, 'td_entry_window_sec', 120):g}с от старта)")
    if not symbols:
