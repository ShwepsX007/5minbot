...
DEFAULTS = {
    ...
    # 0 — метрика выключена. Режим: below — не входим, ЕСЛИ МЕНЬШЕ N%
    # (движение не подтвердилось = флет); above — если БОЛЬШЕ N%
    # (шторм/перегрев — тоже не наша история).
    "td_prev_mode": "below",
    "td_liq_prev_pct": "0",
    "td_cvd_prev_pct": "0",
    "td_oi_prev_pct": "0",
...
def _flat_cfg(c: dict) -> tuple[str, float, float, float]:
    """Настройки фильтра: (режим below|above, порог лика %, порог CVD %, порог OI %)."""
    mode = str(c.get("td_prev_mode", "below") or "below").strip().lower()
    if mode not in ("below", "above"):
        mode = "below"
    return (mode,
            max(0.0, _f_num(c, "td_liq_prev_pct", 0.0)),
            max(0.0, _f_num(c, "td_cvd_prev_pct", 0.0)),
            max(0.0, _f_num(c, "td_oi_prev_pct", 0.0)))


def _flat_enabled(c: dict) -> bool:
    _m, lp, cp, op = _flat_cfg(c)
    return lp > 0 or cp > 0 or op > 0
...
async def _boundary_entry(context, cid, session, c: dict, symbol: str,
    ...
    if _flat_enabled(c) and _flat_cfg(c)[3] > 0:
...
def get_status_text() -> str:
    ...
    _fm, _lp, _cp, _op = _flat_cfg(c)
    if _lp > 0 or _cp > 0 or _op > 0:
        def _mk(name, pct):
            return f"{name} {pct:g}%" if pct > 0 else f"{name} ⛔"
        L.append(f"🌫 Против.prev свечи ({'меньше — не входим' if _fm == 'below' else 'больше — не входим'}): "
                 f"{_mk('лика', _lp)} | {_mk('cvd', _cp)} | {_mk('oi', _op)}")
