...
PARAMS = [
    ...
    ("liq_dyn_thr_on", "📈 Динамический порог по объёму (0/1)", ["0", "1"]),
    ("liq_dyn_thr_hours", "📈 Дин.порог: история, часов", ["6", "12", "24", "48"]),
    ("liq_dyn_thr_min", "📈 Дин.порог: минимум, $", ["50000", "100000", "150000"]),
    ("liq_dyn_thr_max", "📈 Дин.порог: максимум, $", ["300000", "500000", "1000000"]),
    ("liq_dyn_thr_mode", "📈 Дин.порог: режим расчёта", ["minmax", "avg"]),
    ("liq_dyn_thr_src", "📈 Дин.порог: источник объёма", ["all", "self"]),
    ...
    ("liq_loss_liq_on", "🩸 Фильтр убыточной свечи (0-выкл/1-вкл)", ["0", "1"]),
    ("liq_loss_liq_pct", "🩸 Убыточная свеча: порог, %", ["25", "50", "75", "100"]),
    ("liq_loss_liq_mode", "🩸 Убыточная свеча: не входить, если", ["above", "below"]),
    ("liq_pullback_pct", "🌪 Откат свечи: макс. % (0-выкл)", ["0", "25", "35", "50", "60", "75"]),
    ...
    ("liq_exec_retries", "🔁 Попыток входа при тонком стакане", ["1", "2", "3", "5"]),
    ("liq_exec_retry_sec", "🔁 Пауза между попытками, сек", ["3", "5", "10", "15"]),
...
LIQ_PARAM_META = {
    ...
    "liq_threshold_usd": {
        ...
        "hint": ("Порог каскада в $. От 1000 до 10000000. Пример: 75000, 200000. "
                 "Если включён «📈 Динамический порог», это значение используется "
                 "как запасной, когда объёмы торгов не удалось получить."),
    },
    "liq_dyn_thr_on": {
        "type": "enum",
        "allowed": ["0", "1"],
        "hint": (
            "Динамический порог ликвидаций по объёму торгов. Бот берёт часовые "
            "объёмы торгов (USDT) по всем выбранным монетам за N часов, находит "
            "мин/макс и ЛИНЕЙНО растягивает порог между «минимум» и «максимум» "
            "в зависимости от того, где объём последнего часа внутри диапазона. "
            "Активный рынок → порог выше (меньше ложных сигналов), тихий → ниже. "
            "0 — выключен (работает статичный Порог каскада), 1 — включён. "
            "Введи: 1 (вкл) или 0 (выкл)"
        ),
    },
    "liq_dyn_thr_hours": {
        "type": "int",
        ...
        "max": 168,
        "hint": (
            "Сколько часов истории объёмов торгов использовать для расчёта "
            "минимума/максимума/среднего. От 2 до 168 (неделя). Пример: 24"
        ),
    },
    "liq_dyn_thr_min": {
        "type": "int",
        ...
        "max": 10_000_000,
        "hint": (
            "Минимальный порог ликвидаций, $: ставится, когда объём последнего "
            "часа равен минимуму за историю. Пример: 100000"
        ),
    },
    "liq_dyn_thr_max": {
        "type": "int",
        ...
        "max": 10_000_000,
        "hint": (
            "Максимальный порог ликвидаций, $: ставится, когда объём последнего "
            "часа равен максимуму за историю. Пример: 1000000. Если минимум "
            "больше максимума — бот поменяет их местами автоматически."
        ),
    },
    "liq_dyn_thr_mode": {
        "type": "enum",
        "allowed": ["minmax", "avg"],
        "hint": (
            "Как считать динамический порог:\n"
            "minmax (растяжка) — объём последнего часа внутри [мин,макс] "
            "истории линейно растягивает порог между «минимум» и «максимум». "
            "Пример: за 24ч объёмы $10–100млрд, порог $100k–$1M; час $30млрд "
            "→ порог $300k;\n"
            "avg (от среднего) — порог = СЕРЕДИНА диапазона × (объём часа / "
            "средний объём за историю), ограничен [мин,макс]. Активность вдвое "
            "выше средней → порог вдвое выше середины.\n"
            "Введи: minmax (растяжка) или avg (от среднего)"
        ),
    },
    "liq_dyn_thr_src": {
        "type": "enum",
        "allowed": ["all", "self"],
        "hint": (
            "Откуда брать объём торгов:\n"
            "all (общий) — сумма часовых объёмов всех выбранных монет, порог "
            "ОДИН общий для всех монет;\n"
            "self (своя монета) — объём ТОЛЬКО своей монеты: у каждой монеты "
            "свой порог. Масштабы ликвидаций у BTC и, например, XRP сильно "
            "разные, поэтому свой порог на монету точнее.\n"
            "Введи: all (общий) или self (своя монета)"
        ),
    },
    "liq_check_interval": {
        "type": "int",
    ...
    "liq_loss_liq_on": {
        ...
        "allowed": ["0", "1"],
        "hint": (
            "Фильтр ликвидаций УБЫТОЧНОЙ свечи: после убытка бот сравнивает "
            "ОБЪЁМ ликвидаций в убыточном окне с объёмом «сигнального» окна "
            "(того, по которому заходил в рынок) и решает, входить ли снова. "
            "0 — выключен, 1 — включён. Работает для ре-входа после убытка "
            "(шаг серии в режиме «signal»). Пример: 1"
        ),
    },
    "liq_loss_liq_pct": {
        "type": "float",
        ...
        "max": 1000,
        "hint": (
            "Порог N% от объёма ликвидаций СИГНАЛЬНОГО окна. Пример: сигнал "
            "был на $500k, порог 50%. После убытка бот смотрит ликвидации "
            "убыточного окна: $260k = 52% от $500k — больше порога, при "
            "режиме «больше» входа нет; $240k = 48% — меньше порога, вход "
            "есть. Пример: 50"
        ),
    },
    "liq_loss_liq_mode": {
        ...
        "allowed": ["above", "below"],
        "hint": (
            "Когда НЕТ входа после убытка:\n"
            "above (больше) — если ликвидаций в убыточном окне БОЛЬШЕ N% от "
            "сигнального: давление ликвидаций сохраняется, движение может "
            "продолжиться против нас;\n"
            "below (меньше) — если ликвидаций в убыточном окне МЕНЬШЕ N% от "
            "сигнального: поток выдохся, продолжения ждать не стоит.\n"
            "Введи: above (больше) или below (меньше)"
        ),
    },
    "liq_pullback_pct": {
        "type": "int",
        ...
        "max": 100,
        "hint": (
            "Фильтр ОТКАТА сигнальной свечи — только для ПЕРВОЙ сделки "
            "(шаги мартингейла фильтр не касается). Сколько процентов "
            "своего движения свеча может отдать к концу рынка:\n"
            "UP: откат = (high − close) / (high − open);\n"
            "DOWN: откат = (close − low) / (open − low).\n"
            "Откат >= N% — вход отменяется: импульс выдохся, входить в "
            "следующее окно — покупать «хвост». 0 — фильтр выключен. "
            "Пример: 50"
        ),
    },
    ...
    "liq_exec_retries": {
        ...
        "hint": (
            ...
            "(спред/глубина не прошли фильтр): стакан бывает плохим «в "
            "моменте» и восстанавливается за секунды. Между попытками — "
            "пауза «Пауза между попытками». 1 — одна попытка (старое "
            "поведение). Если все попытки неудачны: первичный сигнал "
            "пропускается, а серия мартингейла НЕ останавливается — "
            "следующий сигнал по монете зайдёт тем же лотом. Пример: 3"
        ),
    },
    "liq_exec_retry_sec": {
        "type": "int",
        ...
        "max": 60,
        "hint": (
            "Пауза между попытками входа при тонком стакане, секунд. "
            "Попытки прекращаются досрочно, если окно входа закрывается. "
            "Пример: 5"
        ),
    },
    "liq_max_entry_cents": {
...
def validate_manual_input(key: str, raw_text: str):
    ...
    if key == "liq_loss_liq_mode":
        low = text.lower().strip()
        aliases = {
            "above": "above", "больше": "above", "превышение": "above",
            "выше": "above", "a": "above", "в": "above",
            "below": "below", "меньше": "below", "ниже": "below",
            "затухание": "below", "b": "below", "н": "below",
        }
        matched = None
        for k, v in aliases.items():
            if low == k or low.startswith(k):
        ...
        if matched:
            return True, matched, ""
        return False, "", ("❌ Введи: above (не входить, если ликвидаций "
                           "убыточного окна больше порога) или below (если меньше)")

    if key == "liq_loss_liq_on":
        low = text.lower().strip()
        aliases = {
            "1": "1", "вкл": "1", "включить": "1", "включен": "1",
            "включён": "1", "on": "1", "да": "1",
            "0": "0", "выкл": "0", "выключить": "0", "выключен": "0",
            "выключен": "0", "выключён": "0", "off": "0", "нет": "0",
        }
        matched = None
        for k, v in aliases.items():
            if low == k or low.startswith(k):
        ...
        if matched:
            return True, matched, ""
        return False, "", "❌ Введи: 1 (вкл) или 0 (выкл)"

    if key == "liq_dyn_thr_on":
        low = text.lower().strip()
        aliases = {
            "1": "1", "вкл": "1", "включить": "1", "включен": "1",
            "включён": "1", "on": "1", "да": "1",
    ...
    if key == "liq_dyn_thr_mode":
        low = text.lower().strip()
        aliases = {
            "minmax": "minmax", "минмакс": "minmax", "мин-макс": "minmax",
            "растяжка": "minmax", "stretch": "minmax", "mm": "minmax",
            "avg": "avg", "среднее": "avg", "средн": "avg", "от среднего": "avg",
            "average": "avg", "сред": "avg",
        }
        matched = None
        for k, v in aliases.items():
            if low == k or low.startswith(k):
        ...
        if matched:
            return True, matched, ""
        return False, "", ("❌ Введи: minmax (растяжка между минимумом и "
                           "максимумом) или avg (от среднего объёма)")

    if key == "liq_dyn_thr_src":
        low = text.lower().strip()
        aliases = {
            "all": "all", "общий": "all", "все": "all", "сумма": "all",
            "общ": "all", "a": "all",
            "self": "self", "своя": "self", "свой": "self", "свое": "self",
            "монета": "self", "по монете": "self", "s": "self",
        }
        matched = None
        for k, v in aliases.items():
            if low == k or low.startswith(k):
        ...
        if matched:
            return True, matched, ""
        return False, "", ("❌ Введи: all (общий объём всех монет) или "
                           "self (объём только своей монеты)")
