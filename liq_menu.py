...
LIQ_PARAM_META = {
    ...
    "liq_mg_entry": {
        ...
        "hint": (
            "signal — шаг мартингейла входит в новое окно ТОЛЬКО при "
            "подтверждении: живая свеча окна идёт в нашу сторону или случился "
            "микро-каскад ликвидаций в нашу пользу. Без подтверждения окна "
            "пропускаются (см. настройки ниже).\n"
            "timer — старое поведение: вход сразу по таймеру, вслепую.\n"
    ...
    "liq_mg_skip_windows": {
        ...
        "max": 5,
        "hint": (
            "Сколько окон максимум пропускать, если подтверждения нет. "
            "После исчерпания — вход лотом «после пропусков» или остановка "
            "серии (если лот = 0%). От 0 до 5. Пример: 2"
        ),
    },
    "liq_mg_skip_lot_pct": {
        "type": "float",
        ...
        "max": 100,
        "hint": (
            "Лот после исчерпания пропусков, в % от расчётного. "
            "0 — остановить серию, 100 — войти полным лотом. Пример: 50"
        ),
    },
