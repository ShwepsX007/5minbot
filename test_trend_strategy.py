...
async def main():
    ...
    check("лика(>150) и cvd(<50) не мешают друг другу: реверсит cvd",
          ...
          == stat_c0 + 1)
    scan_note = database.get_setting("td_last_scan", "") or ""
    check("метка ⟵РЕВЕРС стоит на сработавшей метрике (cvd), а не на liq",
          "реверс по метрике «cvd»" in scan_note and "⟵ РЕВЕРС" in scan_note
          and "лика $100/$100: 100.0% от пред. окна (правило: >150%) |"
          in scan_note)
