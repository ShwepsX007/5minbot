...
async def main():
    ...
    await ts._scan_entry_impl(Ctx)
    st = ts._load_state()
    check("лика: 10% < 60% от prev окна → входа нет", SYM not in st["positions"])
    check("лика: окно сожжено (и фолбэк не дублирует)",
          int(st["last_window"].get(SYM, -1)) == w + DUR)
    stat1 = int(float(database.get_setting("td_stat_flat_liq", "0") or 0))
    check("лика: счётчик фильтра вырос", stat1 == stat0 + 1)
    check("лика: причина в td_last_scan",
          "флет" in (database.get_setting("td_last_scan", "") or ""))
    ...
    await ts._scan_entry_impl(Ctx)
    check("лика: 700% ≥ 60% → вход разрешён", SYM in ts._load_state()["positions"])
    ...
    lqa_t.recent_liquidations = liq_events([])
    await ts._scan_entry_impl(Ctx)
    check("никаких ликвидаций 2 окна подряд → блок (0% < 60%)",
          SYM not in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    lqa_t.ws_liquid_ready = lambda: True
    check("WS ликвидаций лежит → фильтр не мешает",
          SYM in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    check("CVD 12.5% < 50% (поток умер) → входа нет",
          SYM not in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    check("CVD 75% ≥ 50% → вход разрешён", SYM in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    check("CVD-стрим лежит → вход не блокируется",
          SYM in ts._load_state()["positions"])
    ...
    check("OI: prev-снапшота нет → не блокируем (первая граница)",
          SYM in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    check("OI 5% < 50% → входа нет (нет новых позиций = флет)",
          SYM not in ts._load_state()["positions"])
    st = ts._load_state()
    ...
    await ts._scan_entry_impl(Ctx)
    check("above: шторм 3000% > 150% → входа нет",
          SYM not in ts._load_state()["positions"])
    ...
    await ts._scan_entry_impl(Ctx)
    check("лика(>150) и cvd(<50) не мешают друг другу: блокирует cvd",
          SYM not in ts._load_state()["positions"]
    ...
    await ts._scan_entry_impl(Ctx)
    check("та же пара фильтров на «всплеске»: блокирует лика, а не cvd",
          SYM not in ts._load_state()["positions"]
    ...
    await ts._scan_entry_impl(Ctx)
    check("унаследованный above: 50% < 150% → вход разрешён",
          SYM in ts._load_state()["positions"])
    ...
    check("статус: строка фильтра появляется при включении",
          (ts.set_param("td_cvd_prev_pct", "50"),
           "против.prev" in ts.get_status_text().lower(),
           ts.set_param("td_cvd_prev_pct", "0"))[1])
    try:
        import bot
